...
function rateLimited($ip) {
  $dir = sys_get_temp_dir() . '/prompter-rate';
...
/* ---------- поля расширенной анкеты ----------
   ...
   рассказывает про город с районами и сайт. В короткой форме этих
   полей нет, и тогда все строки ниже пустые и в сообщение не
   попадают.

   Конкурентов и запросы клиента анкета не спрашивает: конкурентов по
   указанному району проверка находит сама, а если менеджер знает их
   поимённо — дописывает строкой прямо в пересланную заявку.
...
foreach (array(
  ...
  /* В поле «что уже пробовали» человек пишет списком, каждый пункт
     с новой строки. clean() схлопывает переносы в пробелы, и пункты
     слипались бы в одну фразу. Поэтому перенос строки сначала
     превращаем в запятую: по ней список и читается дальше. */

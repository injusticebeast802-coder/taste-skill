...
HELP = (
    ...
    'Проще всего — переслать заявку с сайта целиком, ничего не '
    'перепечатывая: я сам возьму из неё ИНН, название, род занятий '
    'и город.\n\n'
    'Если человек заполнил расширенную анкету, в заявке будут ещё '
    'город с районами и сайт. Сайт я тогда не ищу, а по району сам '
    'найду через поиск, кто работает в этой же нише рядом, и проверю '
    'их поимённо — в том числе тех, кого нейросети не назвали ни разу: '
    'это тоже ответ клиенту. Заодно четверть вопросов задам про район: '
    'людям нужно то, что у дома, и по району нейросеть нередко '
    'называет совсем другие компании.\n\n'
    'Знаете конкурентов клиента или слова, которыми его ищут? Допишите '
    'их в пересланную заявку отдельными строками:\n'
    'Конкуренты: Дента, Белый клык\n'
    'Как ищут: имплантация зубов, детский стоматолог\n\n'
    'Названных конкурентов поставлю на график рядом с клиентом, а его '
    'словами спрошу нейросети — они точнее любой моей заготовки по '
    'отрасли. В анкете на сайте этих полей нет: спрашивать такое у '
    'клиента значит удлинять анкету ради того, что менеджер и так '
    'знает.\n\n'
...
# Обиходные слова, с которых начинается болтовня, а не запрос. Нужны
# потому, что признак запроса теперь — запятая, а «спасибо, всё ок»
# под неё тоже подходит, и бот пошёл бы считать несуществующую
# компанию «спасибо». Ни одна компания так себя не назовёт, а если
# назовёт — её выручит ИНН, он проверяется раньше этого списка.
BOLTOVNYA = {
    'привет', 'здравствуйте', 'здравствуй', 'добрый день', 'добрый вечер',
    'доброе утро', 'доброго дня', 'спасибо', 'спс', 'благодарю',
    'ок', 'окей', 'хорошо', 'ладно', 'отлично', 'супер', 'класс',
    'да', 'нет', 'ага', 'угу', 'понял', 'поняла', 'принял', 'принято',
    'жду', 'давай', 'давайте', 'готово', 'сделал', 'сделано',
    'пока', 'до свидания', 'тест', 'проверка связи',
}
...
def handle(cfg, chat_id, text):
    ...
    # Похоже ли присланное на запрос. Раньше здесь стояло «есть ли
    # в сообщении хоть одна цифра» — сторож времён, когда без ИНН
    # программа работать не умела. Работать научилась, сторож остался,
    # и запросы из заявки он отбивал молча, показывая справку. Причём
    # отбивал и пример из этой же справки: «Flowwow, доставка цветов,
    # Москва» — тоже без единой цифры.
    #
    # Спрашиваем у самого разборщика, чтобы условие здесь не разъезжалось
    # с тем, что программа принимает: годится либо ИНН, либо название
    # и род занятий через запятую. Город не обязателен — он бывает
    # вписан в род занятий словами «барбершоп в Москве».
    inn, brand, kind, _gorod = runner.parse_request(text)
    if not inn and (not (brand and kind) or brand.lower() in BOLTOVNYA):
        send(cfg, chat_id, HELP)
        return

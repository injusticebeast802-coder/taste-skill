...
def _odno_slovo(a, b):
    ...
    Считаем так, если слова совпадают или одно начинается с другого и
    разница в длине невелика. Без второго условия «Котельники-Строй»
    сходился с «Котельники», и название компании принималось за
    название города.
    """
    if a == b:
        return True
    if len(a) < 5 or len(b) < 5:
        return False
    if not (a.startswith(b[:5]) or b.startswith(a[:5])):
        return False
...
def _pohozhe_na_nazvanie(golova, kind, mesta):
    ...
    компания.
    """
    # Название компании короткое. Длинная фраза — это не имя, а
    # описание услуги: «Деревянные дома-бани под ключ в Котельниках».
    # Такое однажды и попало в отчёт, да ещё обрезанным на середине.
    if not (2 <= len(golova) <= 36):
        return False
    slova = golova.lower().replace('«', ' ').replace('»', ' ').split()
    if not slova or len(slova) > 4:
        return False

    # Предлог в конце — верная примета обрезанной фразы: «дома под»,
    # «строительство в». Названий, кончающихся предлогом, не бывает.
    if slova[-1].strip('.,:;') in ('в', 'во', 'на', 'под', 'для', 'из', 'с',
                                   'со', 'по', 'от', 'до', 'у', 'за', 'к'):
        return False
    ...
    for w in slova:
        ...
        #
        # Сравниваем не только по началу, но и по длине. Иначе
        # «Котельники-Строй» опознавался как название города и
        # компания выпадала из отчёта — а это именно компания,
        # названная по своему городу, и таких немало.
        if any(_odno_slovo(w, l) for l in lishnee):
            continue
        return True
    return False

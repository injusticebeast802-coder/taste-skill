...
# Ориентиры роста за три месяца работы, по отраслям. Доля ответов,
# в которых нейросети называют компанию.
#
# Одной цифры на всех тут быть не может, и это не осторожность, а
# устройство рынков. В стоматологии, салоне красоты или автосервисе
# нейросеть спокойно называет небольшую местную компанию: спрашивают
# про район, громких имён в ответе нет, место свободно. В
# строительстве, товарах для дома и детских товарах на общий вопрос
# отвечают ПИК, «Леруа Мерлен» и «Детский мир» — вытеснить их
# небольшой компании не выйдет, и обещать этого нельзя. Там растём в
# своей узкой теме, а не в отрасли целиком, и потолок втрое ниже.
#
# Отрасли — те же, что в разделе «Для кого» на сайте, плюс несколько
# соседних. Темы берём из queries.TRIGGERS, так что править надо в
# двух местах согласованно: тема должна и узнаваться, и иметь строчку
# здесь. Нет строчки — берётся DEFAULT_BAND.
BANDS = {
    # Местные рынки, выбор по району, федеральных имён в ответе нет
    'стоматология': (0.25, 0.40),
    'салон красоты': (0.25, 0.40),
    'медицина': (0.20, 0.35),
    'спорт и фитнес': (0.20, 0.32),
    'юридические услуги': (0.20, 0.32),
    'автосервис': (0.20, 0.30),
    'ремонт техники': (0.20, 0.30),
    'цветы': (0.20, 0.32),
    'грузоперевозки': (0.18, 0.30),
    'реклама и маркетинг': (0.18, 0.30),

    # Рынки посередине: имена на слуху есть, но место рядом с ними есть тоже
    'ремонт и отделка': (0.15, 0.28),
    'обучение': (0.15, 0.28),
    'разработка программ': (0.15, 0.28),
    'розничный магазин': (0.15, 0.28),
    'недвижимость': (0.15, 0.25),
    'гостиница': (0.15, 0.25),
    'оптовая торговля': (0.15, 0.25),
    'ресторан, кафе и доставка еды': (0.15, 0.25),
    'доставка еды': (0.12, 0.22),
    'туризм': (0.12, 0.22),

    # Рынки, забитые федеральными именами
    'строительство': (0.10, 0.20),
    'товары для дома': (0.10, 0.20),
    'детские товары': (0.10, 0.20),
    'интернет-магазин': (0.10, 0.20),
}

# Отрасли, где на общий вопрос отвечают именами федеральных компаний.
# Для них на картинке печатается отдельная строчка: иначе цифры
# выглядят скромно и непонятно почему.
CROWDED = {
    'строительство': 'В строительстве на общий вопрос нейросеть называет крупных '
                     'застройщиков. Растём в вашей теме, а не в отрасли целиком.',
    'товары для дома': 'В товарах для дома на общий вопрос нейросеть называет сети. '
                       'Растём в вашей теме, а не в отрасли целиком.',
    'детские товары': 'В детских товарах на общий вопрос нейросеть называет сети. '
                      'Растём в вашей теме, а не в отрасли целиком.',
    'интернет-магазин': 'На общий вопрос нейросеть называет маркетплейсы. Растём '
                        'в вашей теме, а не в торговле целиком.',
}

DEFAULT_BAND = (0.15, 0.28)

HOLD = 0.85  # выше этого расти уже некуда — говорим про удержание


def band_of(company):
    """Ориентир роста и тема, по которой он выбран."""
    from . import queries

    topic = (queries.topic_of(company.get('industry'))
             or queries.topic_of(company.get('kind')))
    return BANDS.get(topic, DEFAULT_BAND), topic
...
def forecast(data):
    ...
    (lo_s, hi_s), topic = band_of(data['company'])

    # Компанию уже называют чаще, чем ориентир по отрасли: считаем
    # прибавку от того, что есть. Она тем меньше, чем теснее рынок —
    # на забитом рынке каждый следующий ответ даётся тяжелее.
    if share >= lo_s:
        lo_s = min(share + lo_s * 0.4, 0.90)
        hi_s = min(share + hi_s * 0.5, 0.95)
...
def _draw_forecast(d, data, f, b, top, W, PAD, f_h2, f_xs):
    ...
    why_lines = _wrap(d, f.get('why') or '', f_xs, bar_w)[:2]
    body_lines = _wrap(d, body, f_xs, bar_w)[:3]
    n_extra = ((1 if f['gain'] else 0) + len(why_lines)
               + (1 if s_line else 0) + len(body_lines))
    ...
    for line in why_lines:
        d.text((bar_x, ty), line, font=f_xs, fill=b['c2'])
        ty += 24
    if s_line:
